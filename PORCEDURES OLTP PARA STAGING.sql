...
ALTER PROCEDURE SP_OLTP_FAIXAETARIA(@DATACARGA DATETIME) AS
BEGIN
...
BEGIN
	...
	WHILE (@@FETCH_STATUS = 0)
		BEGIN
		DECLARE @COD_DATA_ENTRADA INT, @COD_DATA_SAIDA INT, @COD_PACOTE INT, @COD_EMPRESA INT
		...
		SELECT @COD_PACOTE = m.PAC_Codigo,
			   @COD_EMPRESA = m.ACA_Codigo FROM TB_Mensalidade m
												INNER JOIN  TB_Pacote p ON (m.PAC_Codigo = p.PAC_Codigo)
												WHERE m.ALU_Codigo = @CODIGO_ALUNO
		...
		INSERT INTO TB_AUX_FLUXO(DATA_CARGA,FLU_Codigo,FLU_CodigoData,FLU_HorarioEntrada,FLU_CodAluno,FLU_CodPacote,FLU_CodEmpresa,FLU_CodFaixaEtaria,FLU_CodLocalizacao,FLU_CodTurno)
									VALUES (@DATACARGA,@CODIGO_FLUXO,@COD_DATA_ENTRADA,@COD_HORARIO_ENTRADA,@CODIGO_ALUNO,@COD_PACOTE,@COD_EMPRESA,@CODIGO_FAIXA_ETARIA,@END_CODIGO,@TUR_CODIGO)         
		...
		INSERT INTO TB_AUX_FLUXO(DATA_CARGA,FLU_Codigo,FLU_CodigoData,Flu_HorarioSaida,FLU_CodAluno,FLU_CodPacote,FLU_CodEmpresa,FLU_CodFaixaEtaria,FLU_CodLocalizacao,FLU_CodTurno)
								VALUES (@DATACARGA,@CODIGO_FLUXO,@COD_DATA_SAIDA,@COD_HORARIO_SAIDA,@CODIGO_ALUNO,@COD_PACOTE,@COD_EMPRESA,@CODIGO_FAIXA_ETARIA,@END_CODIGO,@TUR_CODIGO)         
